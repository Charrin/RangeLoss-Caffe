    ...
    if (propagate_down[i]) {
        ...
        if (loss_inter > 0) {
            ...
            for (int c = 0; c < map_class[class1].size(); ++c) {
		        ...
		        caffe_cpu_axpby(K_, (-2*inter_weight_)/(map_class[class1].size()), distance_data, Dtype(1.), bottom_diff + ind * K_);
            }

            // class2
            for (int c = 0; c < map_class[class2].size(); ++c) {
                int ind = map_class[class2][c];
		        caffe_cpu_axpby(K_, 2*inter_weight_/(map_class[class2].size()), distance_data, Dtype(1.), bottom_diff + ind * K_);
            }
            LOG(INFO) << "class num " << map_class[class1].size() << " " << map_class[class2].size();
        }
